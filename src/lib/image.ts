...
/**
 * Guesses the grid's quad to seed the corner-drag UI.
 *
 * Keying on *long* runs of ink rather than ink in general is what makes this
 * usable on a real photo: the page edge, the shadowed background, the header
 * text and the magazine logo all disappear, because only the grid's own rules
 * span a large fraction of the frame.
 */
export function guessGridQuad(bin: BinaryImage): Quad {
  const { width: w, height: h, data } = bin
  const minRunH = Math.round(w * 0.3)
  const minRunV = Math.round(h * 0.3)

  const colInk = new Int32Array(w)
  const rowInk = new Int32Array(h)

  // Horizontal long runs.
  for (let y = 0; y < h; y++) {
    const row = y * w
    let start = -1
    for (let x = 0; x <= w; x++) {
      const ink = x < w && data[row + x] === 1
      if (ink) {
        if (start < 0) start = x
      } else if (start >= 0) {
        if (x - start >= minRunH) {
          rowInk[y]! += x - start
          for (let k = start; k < x; k++) colInk[k]!++
        }
        start = -1
      }
    }
  ...
  for (let x = 0; x < w; x++) {
    ...
    for (let y = 0; y <= h; y++) {
      ...
      if (ink) {
        ...
        if (y - start >= minRunV) {
          colInk[x]! += y - start
          for (let k = start; k < y; k++) rowInk[k]!++
        }
        start = -1
      }
    }
  ...
  const span = (profile: Int32Array, limit: number, fallbackLo: number, fallbackHi: number) => {
    let total = 0
    for (let i = 0; i < limit; i++) total += profile[i]!
    if (total === 0) return [fallbackLo, fallbackHi] as const
    // Trim a sliver of ink at each end so a single stray rule cannot drag the
    // box out to the page border.
  ...
  const inset = 0.02
  const [x0, x1] = span(colInk, w, Math.round(w * inset), Math.round(w * (1 - inset)))
  const [y0, y1] = span(rowInk, h, Math.round(h * inset), Math.round(h * (1 - inset)))
  return [
    { x: x0, y: y0 },
    { x: x1, y: y0 },
    { x: x1, y: y1 },
